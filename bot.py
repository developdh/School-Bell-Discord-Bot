...
async def notify_transition(cid: int, name: str, mode: str) -> None:
    ch = await _get_channel(cid)
    if ch:
...
async def notify_break_event(gs: dict, brk: dict, end_ts: float, extending: bool) -> None:
...
async def notify_resume(gs: dict) -> None:
    for ch in await _break_channels(gs):
            ...
            async with lock:
                ...
                for brk in gs["breaks"]:
                    ...
                    if not already or gs["pause_until"] < end_ts:
                        ...
                        await notify_break_event(gs, brk, end_ts, already)
                    # 다음 날 스케줄로 갱신
                ...
                if gs["pause_until"] is None:
                    for name, t in list(gs["timers"].items()):
                        if ts >= t["phase_end_at"]:
                            ...
                            await notify_transition(t["channel_id"], name, new_mode)
...
def parse_command(raw: str) -> list[dict]:
    """
    공백 토큰 기반 왼쪽부터 순차 파싱.
    반환: 액션 리스트 (type: status | stop | break | timer)
    """
